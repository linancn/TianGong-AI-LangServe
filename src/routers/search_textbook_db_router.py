...
from src.services.standalone import search_textbook_db
...
async def search_textbook(request: SearchRequest):
    """
    This endpoint allows you to perform hybrid search (semantic and key word search) on the textbook database for precise and specialized information.
    ...
    """
    try:
        result = await search_textbook_db.search(
            query=request.query,
            top_k=request.top_k,
            ext_k=request.ext_k,
        )
