...
from src.services.standalone import search_report_db
...
async def search_report(request: SearchRequest):
    """
    This endpoint allows you to perform hybrid search (semantic and key word search) on the report database for precise and specialized information.
    ...
    """
    try:
        result = await search_report_db.search(
            query=request.query,
            top_k=request.top_k,
            ext_k=request.ext_k,
        )
